...
def main(args):
    ...
    if args.revshell:
        if args.cmd:
            print("This tool cannot peform both revshell and cmd at same time. You must choose one.")
            exit()
        if args.lhost:
            if args.lport:
                lhost = args.lhost
            ...
            exit()
        if args.verbose:
            print(f"Entering revshell mode. Revshell will be generated for {lhost}:{lport}")
        mode = "revshell"
    ...
    if args.cmd:
        if args.revshell:
            ...
            exit()
        if args.verbose:
            print(f"Entering cmd mode. Your command is {args.cmd}. A payload using the WinExec api call will be made.")
        mode = "cmd"
        arguments = args.cmd 
    ...
    if args.test:
        ...
        if args.format == 0:
            ...
            print("You have chosen an invalid format, test files can only be created for keystone, python, csharp and c")
            exit()
        if args.verbose:
            print(f"You have chosen to create a test file called {filename} using format {format}")
...
if __name__  == "__main__" :
    ...
    parser.add_argument("-r", "--revshell", help = "Create a revshell, must provide an ip address and a port", action = "store_true")
    ...
    parser.add_argument("-c", "--cmd", help = "Use the WinExec API call to execute a command")
    parser.add_argument("-o", "--output", help = "Write the shellcode to a file. Format can be specified with -f. The default format is python", default = 0)
    parser.add_argument("-f", "--format", help = formathelp, default = 0)
    parser.add_argument("-t", "--test", help = "Output a file that can be used to test the payload in the specified format. If not filename is given, the default is test.txt and the format is keystone.", action = "store_true")
